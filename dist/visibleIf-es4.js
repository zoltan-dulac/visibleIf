...
var visibleIf = new function () {
  ...
  me.init = function (reset) {
    ...
    setMandatoryStates();
    setEvents(); //strutsHelpers.populateDynamicFormElements();
  };
          ...
          for (var j = 0; j < inputVars.length; j++) {
            ...
            if (!nodesWithEventsAttached[parentForm.id] || !nodesWithEventsAttached[parentForm.id][inputVar]) {
              ...
              if (inputVars.length > 0) {
                var fieldNode = parentForm[inputVars[j]]; //var fieldNode = document.getElementById(inputVars[j]);
  ...
  function setMandatoryState(node) {
    ...
    if (isRuleTrue) {
      nodeClassList.add('mandatoryIf-mandatory'); //setFormElementsInside(node, false);
    } else {
      nodeClassList.remove('mandatoryIf-mandatory'); //setFormElementsInside(node, true);
    }
  }

  function evaluateRule(parentForm, rule) {
    //node, ruleNode) {
    ...
    if (rule == "") {
      ...
      if (rule != null) {
        ...
        stringToEval = diddledRule.replace(leftBkRe, '( ').replace(rightBkRe, ' )').replace(varRe, 'getFieldValue(document.getElementById("' + formId + '")["$1"]) ').replace(reRe, '.match(/$1/)'); // now, replace placeholders back to the original strings.
        ...
        switch (el.nodeName) {
            ...
            switch (el.type) {
              case "checkbox":
                if (el.checked) {
                  ...
                  } else if (formCache.getValue(el.name) == el.value) {//el.checked = true;
                  }
                }
              ...
              default:
                ...
                } else {//el.value = formCache.getValue(el.name);
                }
          ...
          case "TEXTAREA":
            if (doClear) {
              ...
              if (options.isPageLoad) {//el.value = DOMHelpers.getAttributeValue(el, 'value');
              } else if (!elClassList.contains('visibleIf-doNotReset')) {
                el.value = "";
                addToInputToClear(el);
              }
          ...
          case "SELECT":
            if (doClear) {
              if (!elClassList.contains('visibleIf-doNotReset')) {
                // cache the value
                formCache.setValue(el.name, el.selectedIndex); // TODO: should this be 0 or -1?
  ...
  if (window.XMLHelpers) {
    ...
    XMLHelpers = new function () {
      ...
      me.getXMLHttpRequest = function (url, processReqChange) //, method, data, isAsync)
      {
        ...
        var req; // branch for native XMLHttpRequest object

        if (window.XMLHttpRequest) {
          req = new XMLHttpRequest(); // no support branch
        } else {
          return null;
        }
        ...
        req.open(httpMethod, url, isAsync); //Fixes IE Caching problem
